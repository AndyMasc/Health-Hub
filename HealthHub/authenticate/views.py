...
from .forms import AddPatientForm, AuthorizeUser, CreateUserForm, UpdateUser
    ...
    if request.method == "POST":
        ...
        if patient_obj is not None:
            try:
                ...
                messages.error(
                    request,
                    "This user is not registered as a patient. Register a valid patient account.",
                )
...
def remove_patient(request, patient_id):
    ...
    if (request.user.account.role == "Doctor") and patient.doctor.filter(
        user=request.user
    ).exists():
